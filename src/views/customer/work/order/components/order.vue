<template>
  ...
  <el-card class="work-order-card" shadow="hover">
    ...
    <el-form
      :disabled="$route.query.markId == 110 ? true : false"
      ref="form"
      size="mini"
      class="form"
      :rules="rules"
    ...
    >
      ...
      <el-form-item label="受理人" prop="receiverNo">
        <el-select v-model="form.receiverNo" placeholder="请选择">
          <el-option v-for="item in staff" :key="item.id" :label="`${item.userName}${item.currentLine==1?'(在线)':''}`"
    ...
    <!-- 选择工单类型 -->
    <el-dialog title="选择工单类型" :visible.sync="dialogVisible" width="50%" :append-to-body="true">
				...
				this.$post("/data/my", {
					...
					if (res.state == 0) {
						if (res.data.page.result) {
							that.department = res.data.page.result;
						}
					}
					else {
