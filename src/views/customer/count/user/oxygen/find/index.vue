...
* author: liumengya
* created: 2018/10/9
* describe:统计分析/用户统计/血氧测量统计/详情
-->
<template>
    <div class="oxygen-find">
      <div class="title">
        <p></p>
      ...
      <div class="title" style="border-top: 1px solid #ccc">
        ...
        <div class="table" ref="table" v-loading="loading" element-loading-text="拼命加载中">
            <el-table :data="data" border stripe style="width: 100%;">
                <el-table-column prop="spo2" label="SpO2%" align="center">
                    <template slot-scope="scope">
                        {{scope.row.spo2+' %'}}
                    </template>
                </el-table-column>
                <el-table-column prop="bpm" label="BPM" align="center">
                    <template slot-scope="scope">
                        <p>
                            <span>{{scope.row.bpm}}</span>
                            <span>（{{scope.row.bpmLevel}}）</span>
                        </p>
                    </template>
                </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.loading = true;
      this.$get("/data/my", {
        pathL: "/health/bloodOxygen/queryAllMeasureByUserId",
