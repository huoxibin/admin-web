      ...
      <el-table
        ...
        >
          <template slot-scope="scope">
            {{ scope.row.measuringTime }}
            <i class="el-icon-top b symbol-start" v-if="scope.row.centigradeLevel!='正常'"></i>
          </template>
        </el-table-column>
      </el-table>
	...
	export default {
		...
		methods: {
			...
			getDetail () {
				let that = this;
				this.$get('/data/my', {
					pathL: '/health/bodyTemperature/details',
					userId: this.$route.query.id,
					pageNum: this.recordPage.pageNum,
					pageSize: 10,
				}).then(res => {
					if (res.state == 0) {
						that.dealTableData(res.data.result.list);
