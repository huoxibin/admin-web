<template>
  <div class="update" v-loading="loading" element-loading-text="拼命加载中">
    <el-form ref="form" size="mini" :rules="formRules" style="width:100%" :model="form" label-width="100px">
      <div class="update-top">
        <div class="update-top-left">
          <el-form-item label="标题" prop="topical">
            <el-input v-model="form.topical" placeholder="请输入测试题目的标题"></el-input>
          </el-form-item>
          <el-form-item label="描述" prop="description">
            <el-input v-model="form.description" type="textarea" placeholder="选填，请输入测试题目的描述"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="orderNumber">
            <el-input v-model="form.orderNumber" placeholder="数字越大，在前端排序越靠前"></el-input>
          </el-form-item>
        ...
        <div class="update-top-right">
          <el-form-item label="封面" prop="imageUrl">
            <div class="upload-img" v-loading="loadingPic" element-loading-text="正在上传中···">
              <el-upload class="avatar-uploader" :headers="uploadHeaders" :action="uploadUrl" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
      ...
      <div class="update-bottom">
        <el-card class="box-card">
          ...
          <el-card v-for="item,index in form.subject" :key="index" class="box-card" style="margin-top:10px;" shadow="hover">
            <el-form ref="item" size="mini" :rules="itemRules" style="width:100%" :model="item" label-width="100px">
              <el-form-item label="序号设置">
                <div class="number">
                  <el-input style="width:400px" disabled v-model="item.sortNumber"></el-input>
                  <ul>
                    <li @click="add(1)"><img src="./../images/icone-1.png" alt=""></li>
                    <li @click="remove(1,index)"><img src="./../images/icone-2.png" alt=""></li>
                    <li @click="upData(1,index)"><img src="./../images/icone-3.png" alt=""></li>
                    <li @click="downData(1,index)"><img src="./../images/icone-4.png" alt=""></li>
                  </ul>
                </div>
              </el-form-item>
              ...
              <el-form-item label="选项设置">
                <div class="tables">
                  ...
                  <el-form v-for="itemer,indexer in item.options" :key="indexer" ref="itemer" size="mini" :rules="itemerRules" style="width:100%" :model="itemer">
                    <ol>
                      <li>
                      ...
                      <li style="border-right:none;">
                        <el-form-item style="margin:18px;width:100%">
                          <div class="table-btn">
                            ...
                            <img @click="downData(2,indexer,index)" src="./../images/icone-4.png" alt="">
                          </div>
                        </el-form-item>
                      </li>
                    </ol>
        ...
        <el-card class="box-card" style="margin-top:20px;">
          ...
          <div class="tables">
            ...
            <el-form v-for="itemest,indexest in form.answerrule" :key="indexest" ref="itemest" size="mini" :rules="itemestRules" style="width:100%" :model="itemest">
              <ol>
                <li>
                  <el-form-item style="margin-top:18px;" prop="sortNumber">
                    {{itemest.sortNumber}}
                  </el-form-item>
                </li>
                ...
                <li style="flex:3">
                  <el-form-item style="margin:18px;width:90%" prop="resultDesc">
                    <el-input type="textarea" maxlength="1000" resize="none" v-model="itemest.resultDesc"></el-input>
                  </el-form-item>
                </li>
                <li style="border-right:none;">
                  <el-form-item style="margin:18px;width:100%">
        ...
        <el-card class="box-card" style="margin-top:20px;" shadow="hover">
          ...
          <el-form-item label=''>
            <el-radio-group v-model="resource">
              ...
              <el-radio :label="2">
                总得分乘以系数 ×
                <el-input size="mini" :disabled="resource !== 2 " placeholder="请输入数字及小数点，小数点后只能输入2位" style="width: 85%;margin-left: 20px" v-model="form.scoreRate"></el-input>
              </el-radio>
            </el-radio-group>
          </el-form-item>
    ...
    <el-dialog title="提示" :append-to-body="true" :visible.sync="dialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="mini" @click="updateSubmit(form,0)">否</el-button>
        <el-button size="mini" type="primary" @click="updateSubmit(form,1)">是</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
var CheckScoreScoreMin = (rule, value, callback) => {
  if (!value && value !== 0) {
    callback(new Error("请输入正确的分数"));
  } else if (!isValidNumScore(value)) {
    callback(new Error("请输入0-99999之间的数字"));
...
export default {
  ...
  watch: {
    resource(now) {
      this.form.scoreRate = now === 1 ? "" : this.form.scoreRate;
    }
  },
  methods: {
    // 获取栏目信息
      ...
      if (this) {
        ...
        this.$post(
          ...
          this.$qs.stringify({
            ...
            scoreRate: this.resource === 1 ? 1 : Number(this.form.scoreRate)
          })
        ).then(res => {
          this.loading = false;
          if (res.state === 0) {
    ...
    submit(form) {
      ...
      //新增题目中的ref = ‘item’中（选项设置）的（ref = 'itemer'）
      ...
      for (let m = 0; m < form.answerrule.length; m++) {
        ...
        // console.log(Number(form.answerrule[m].scoreMin));
        // console.log(Number(form.answerrule[m].scoreMax));
        if (
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          if (flag) { //新增题目中的ref = ‘item’
            if (flager) { //新增题目中的ref = ‘item’中（选项设置）的（ref = 'itemer'）
              if (flagest) { //题目规则（answerrule）---中（ref="itemest"）
                if (score) { //题目规则（answerrule）---中（ref="itemest"） 中的分数比较
