          ...
          >
            ...
            <el-table-column label="文章标题" show-overflow-tooltip align="center">
              <template slot-scope="scope">
                ...
                >{{scope.row.businessTitleRtn}}
                </el-button>
              </template>
            </el-table-column>
            <el-table-column prop="doctorNameRtn" label="评论用户" align="center"></el-table-column>
            <el-table-column label="评论内容" align="center">
              <template slot-scope="scope">
                <el-popover
                  placement="top"
                  trigger="hover"
                  :content="scope.row.commentsContentBefore"
                >
                  <p slot="reference">{{scope.row.commentsContent}}</p>
                </el-popover>
              </template>
            </el-table-column>
            <el-table-column prop="createTimeRtn" label="评论时间" align="center"></el-table-column>
            <el-table-column label="评论状态" align="center">
              <template slot-scope="scope">
                <p v-if="scope.row.commentStatusRtn === 2">通过</p>
                <p v-else>屏蔽</p>
              </template>
            </el-table-column>
            <el-table-column label="用户状态" align="center">
              <template slot-scope="scope">
                <p v-if="scope.row.userStatusRtn === '--'">{{ scope.row.userStatusRtn }}</p>
                <p v-else style="color: #f00;">{{ scope.row.userStatusRtn }}</p>
              </template>
            </el-table-column>
...
<script>
	...
	export default {
		...
		data: () => ({
			form: {
				pathL: "/doctor/commentsCheck/getCommentsList",
				businessTitle: "",
				doctorName: "",
				commentsCotent: "",
				commentsStatus: "",
      ...
      this.$nextTick(()=>{
		...
		methods: {
			...
			// 预览文章
			preview (row) {
				const businessTypeRtn = row.businessTypeRtn;
				const path = [
					"",
					"/news/health/list/find",
					"/news/case/preview/app",
					"/commodity/course/detail"
				];
				this.$router.push({
					path: path[ businessTypeRtn ],
					query: {
						id: row.businessIdRtn
					}
				});
			},
			// 点击选择框
					...
					if (date > thirty) {
						...
						const selection = this.selection.map(item => {
							return {
								...
								forbidDateTime,
								mainCommentIdRtn: item.mainCommentIdRtn,
								commemtId: item.commemtId
							};
						});
						this.$post("/data/my", {
							pathL: "/doctor/commentsCheck/setDoctorStatus",
							setDoctorStatus: JSON.stringify(selection)
						}).then(res => {
							if (res.state === 0) {
								this.status = null;
								this.$message({
			...
			solution (type, row) {
				this.$confirm("此操作将解禁该用户, 是否继续?", "提示", {
					...
					.then(() => {
						...
						this.$post("/data/my", {
							pathL: "/doctor/commentsCheck/deleteDoctorForbiden",
			...
			handleStatus (type, status, row) {
				this.$confirm(
					...
					.then(() => {
						...
						this.$post("/data/my", {
							pathL: "/doctor/commentsCheck/setCommentsStatus",
