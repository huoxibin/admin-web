<template>
  <div class="add" v-loading="loading" element-loading-text="拼命加载中">
    <!-- title -->
    <div class="addTitle">修改{{title}}</div>
    <!-- 填写文章表单 -->
    <div class="textForm">
      <el-form
        style="width:800px;"
      ...
      >
        ...
        <el-form-item label="Tag词" prop="tagStr">
          <div class="liulan" @click="openTags">浏览</div>
          <div class="eltagShow">
              ...
              <el-tabs style="height:100%" v-model="activeName">
                ...
                <el-tab-pane label="资源库" name="second">
                  ...
                  <div class="server_btn">
                    <el-button @click="serverImgOK" size="mini" type="primary">确定</el-button>
                    <el-button @click="ydtShow=false" size="mini" type="info" plain>取消</el-button>
                  </div>
                </el-tab-pane>
              </el-tabs>
        ...
        <div style="display:flex;" v-if="commentShow">
          <el-form-item label="评论开始时间" required>
            <el-date-picker
              style="width:600px"
              v-model="commentdate"
              type="daterange"
        ...
        <el-form-item v-if="id==='2' || id===2" prop="context">
          <el-button @click="uploadImgs" type="primary">
          ...
          <div class="photoList" v-for="(item,index) in uploadImgGrouper" :key="index">
            ...
            <div class="photoList_right">
              <el-form
                ...
                label-width="80px"
              >
                <!--:model="item" :rules="imgFromRules" :model="item.imgFrom"-->
                <el-form-item label="标题" prop="title">
        ...
        <el-form-item v-if="id==='3' || id===3" v-show="videoShow && videoType === 1">
        ...
        <!-- 上传音频 -->
        <el-form-item v-if="id==='4' || id===4" prop="context">
        ...
        <!-- 阅读量的增长数 -->
        <el-form-item label="阅读量增长数" prop="readStep">
        ...
        <!-- 语音播放按钮 -->
        <el-form-item v-if="$route.query.contextType == 1" label="语音播放" prop="favoriteIncreaseNum">
        ...
        <!-- 提交表单 -->
        <el-form-item>
          <el-button type="primary" @click="submitForm(textForm,item)">保存</el-button>
          <el-button @click="back">取消</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div v-if="updateloading" class="loading" v-loading="updateloading" element-loading-text="正在保存"></div>
  </div>
</template>
	...
	export default {
		...
		watch: {
			videoUrl (now) {
				...
				else if (this.videoType === 2 && now !== null) {
		...
		methods: {
			...
			// 提交表单
			submitForm (textForm) {
				if (this.commentdate === null || this.commentdate === undefined) {
					this.commentdate = "";
				}
				...
				this.$refs.textForm.validate(valid => {
					if (valid) {
						if (iT) {
							...
							var isStatus = null;
							if (textForm.status === 4) {
								isStatus = this.$qs.stringify({
									...
									commentBdate: this.commentdate[ 0 ],
									commentEdate: this.commentdate[ 1 ],
									...
									favoriteIncreaseNum: textForm.favoriteIncreaseNum,
									defaultFavoriteAmout: textForm.defaultFavoriteAmout,
			...
			serverTree (index, data) {
				...
				this.$axios
					.get("/data/my", {
						params: {
							pathL: "/ap/newsArticle/sourceFiles",
			...
			flyes () {
				...
				this.terminalChannel = treeNames.join(",");
				this.channelIds = treeIds.join(",");
				this.flShow = false;
			}
		},
		mounted () {
			...
			this.$axios
				...
				.then(res => {
					that.loading = false;
					that.textForm = res.data.data.articleInfo;
					that.$nextTick(() => {
						that.textForm.radioPlay = that.textForm.radioPlay == 1 ? true : false;
					})
					that.videoType = that.textForm.videoType;
					if (
						res.data.data.articleInfo.commentBdate !== null &&
						res.data.data.articleInfo.commentEdate !== null
					) {
						let commentBdate = res.data.data.articleInfo.commentBdate;
						let commentEdate = res.data.data.articleInfo.commentEdate;
						this.commentdate = [ commentBdate, commentEdate ];
					}
					this.id = res.data.data.articleInfo.contextType;
					if (res.data.data.imageList) {
						if (res.data.data.imageList.length !== 0) {
							this.uploadImgGrouper = res.data.data.imageList.reverse();
						}
					}
					var tagArray = res.data.data.articleInfo.tagStr;
					if (tagArray !== "") {
						this.tagStr = tagArray.split(",");
						this.checkboxGroup = tagArray.split(",");
					}
					this.channelIds = res.data.data.articleInfo.channelIds;
					this.textForm.channelIds = res.data.data.articleInfo.channelIds;
					this.terminalChannel = res.data.data.terminalChannel;
		...
		updated () {
			if (
				this.$refs.articleType &&
				this.$refs.articleType.value &&
				this.$refs.articleType.value == 2
			) {
				this.zzShow = true;
			}
			else {
			...
			if (
				this.$refs.isAllowComment &&
				this.$refs.isAllowComment.value &&
				this.$refs.isAllowComment.value == 1
			) {
